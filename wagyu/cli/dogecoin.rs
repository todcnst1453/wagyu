...
impl DogecoinWallet {
    pub fn new<N: DogecoinNetwork, R: Rng>(rng: &mut R, format: &DogecoinFormat) -> Result<Self, CLIError> {
        let private_key = DogecoinPrivateKey::<N>::new(rng)?;
    ...
    pub fn new_hd<N: DogecoinNetwork, W: DogecoinWordlist, R: Rng>(
        rng: &mut R,
        word_count: u8,
        password: Option<&str>,
        path: &str,
    ) -> Result<Self, CLIError> {
        let mnemonic = DogecoinMnemonic::<N, W>::new_with_count(rng, word_count)?;
        let master_extended_private_key = mnemonic.to_extended_private_key(password)?;
        let derivation_path = DogecoinDerivationPath::from_str(path)?;
    ...
    pub fn from_mnemonic<N: DogecoinNetwork, W: DogecoinWordlist>(
        mnemonic: &str,
        password: &Option<&str>,
        path: &str,
    ) -> Result<Self, CLIError> {
        let mnemonic = DogecoinMnemonic::<N, W>::from_phrase(&mnemonic)?;
    ...
    pub fn from_extended_private_key<N: DogecoinNetwork>(
        extended_private_key: &str,
        path: &Option<String>,
    ) -> Result<Self, CLIError> {
        let mut extended_private_key = DogecoinExtendedPrivateKey::<N>::from_str(extended_private_key)?;
        if let Some(derivation_path) = path {
            let derivation_path = DogecoinDerivationPath::from_str(&derivation_path)?;
    ...
    pub fn from_extended_public_key<N: DogecoinNetwork>(
        extended_public_key: &str,
        path: &Option<String>,
    ) -> Result<Self, CLIError> {
        let mut extended_public_key = DogecoinExtendedPublicKey::<N>::from_str(extended_public_key)?;
        if let Some(derivation_path) = path {
            let derivation_path = DogecoinDerivationPath::from_str(&derivation_path)?;
    ...
    pub fn from_private_key<N: DogecoinNetwork>(private_key: &str, format: &DogecoinFormat) -> Result<Self, CLIError> {
        let private_key = DogecoinPrivateKey::<N>::from_str(private_key)?;
        let public_key = private_key.to_public_key();
        let address = public_key.to_address(format)?;
        Ok(Self {
            private_key: Some(private_key.to_string()),
    ...
    pub fn from_public_key<N: DogecoinNetwork>(public_key: &str, format: &DogecoinFormat) -> Result<Self, CLIError> {
        let public_key = DogecoinPublicKey::<N>::from_str(public_key)?;
        let address = public_key.to_address(format)?;
        Ok(Self {
            public_key: Some(public_key.to_string()),
            address: Some(address.to_string()),
    ...
    pub fn from_address<N: DogecoinNetwork>(address: &str) -> Result<Self, CLIError> {
        let address = DogecoinAddress::<N>::from_str(address)?;
        Ok(Self {
            address: Some(address.to_string()),
            network: Some(N::NAME.to_string()),
            format: Some(address.format().to_string()),
    ...
    pub fn to_signed_transaction<N: DogecoinNetwork>(
        transaction_hex: &str,
        inputs: &Vec<DogecoinInput>,
    ) -> Result<Self, CLIError> {
        let mut transaction = DogecoinTransaction::<N>::from_transaction_bytes(&hex::decode(transaction_hex)?)?;
            ...
            match (input.amount.clone(), input.address.clone(), input.private_key.clone()) {
                (Some(amount), Some(address), Some(private_key)) => {
                    ...
                    let outpoint = Outpoint::<N>::new(
                        ...
                        input.vout,
                        Some(address),
                        Some(DogecoinAmount::from_satoshi(amount as i64)?),
                        redeem_script,
                        script_pub_key,
                    )?;
...
impl Display for DogecoinWallet {
...
/// Represents options for a Dogecoin wallet
#[derive(Clone, Debug, Serialize)]
pub struct DogecoinOptions {
    // Standard command
    count: usize,
...
impl Default for DogecoinOptions {
    fn default() -> Self {
        Self {
            // Standard command
            count: 1,
...
impl CLI for DogecoinCLI {
    ...
    /// Handle all CLI arguments and flags for Dogecoin
    #[cfg_attr(tarpaulin, skip)]
    fn parse(arguments: &ArgMatches) -> Result<Self::Options, CLIError> {
        let mut options = DogecoinOptions::default();
    ...
    /// Generate the Dogecoin wallet and print the relevant fields
    #[cfg_attr(tarpaulin, skip)]
    fn print(options: Self::Options) -> Result<(), CLIError> {
        fn output<N: DogecoinNetwork, W: DogecoinWordlist>(options: DogecoinOptions) -> Result<(), CLIError> {
            let wallets =
                match options.subcommand.as_ref().map(String::as_str) {
                    Some("hd") => match options.to_derivation_path(true) {
                        Some(path) => (0..options.count)
                            .flat_map(|_| {
                                match DogecoinWallet::new_hd::<N, W, _>(
                    ...
                    Some("import-hd") => {
                        if let Some(mnemonic) = options.mnemonic.clone() {
                            ...
                            match options.to_derivation_path(true) {
                                Some(path) => vec![DogecoinWallet::from_mnemonic::<N, ChineseSimplified>(
                                    &mnemonic, password, &path,
                                )
                                .or(DogecoinWallet::from_mnemonic::<N, ChineseTraditional>(
                                    &mnemonic, password, &path,
                                ))
                                .or(DogecoinWallet::from_mnemonic::<N, English>(&mnemonic, password, &path))
                                .or(DogecoinWallet::from_mnemonic::<N, French>(&mnemonic, password, &path))
                                .or(DogecoinWallet::from_mnemonic::<N, Italian>(&mnemonic, password, &path))
                                .or(DogecoinWallet::from_mnemonic::<N, Japanese>(&mnemonic, password, &path))
                                .or(DogecoinWallet::from_mnemonic::<N, Korean>(&mnemonic, password, &path))
                                .or(DogecoinWallet::from_mnemonic::<N, Spanish>(&mnemonic, password, &path))?],
                                None => vec![],
                            }
                    ...
                    _ => (0..options.count)
                        .flat_map(
                            |_| match DogecoinWallet::new::<N, _>(&mut StdRng::from_entropy(), &options.format) {
                                Ok(wallet) => vec![wallet],
                                _ => vec![],
                            },
                        )
