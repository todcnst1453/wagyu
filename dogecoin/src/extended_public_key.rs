...
use crate::extended_private_key::DogecoinExtendedPrivateKey;
...
/// Represents a Dogecoin extended public key
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DogecoinExtendedPublicKey<N: DogecoinNetwork> {
    /// The address format
    ...
    chain_code: [u8; 32],
    /// The Dogecoin public key
    public_key: DogecoinPublicKey<N>,
}

impl<N: DogecoinNetwork> ExtendedPublicKey for DogecoinExtendedPublicKey<N> {
    type Address = DogecoinAddress<N>;
    type DerivationPath = DogecoinDerivationPath<N>;
    type ExtendedPrivateKey = DogecoinExtendedPrivateKey<N>;
...
impl<N: DogecoinNetwork> DogecoinExtendedPublicKey<N> {
...
impl<N: DogecoinNetwork> FromStr for DogecoinExtendedPublicKey<N> {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let format = DogecoinFormat::from_extended_public_key_version_bytes(&data[0..4])?;
        ...
        let public_key = DogecoinPublicKey::from_secp256k1_public_key(secp256k1_public_key, true);
...
impl<N: DogecoinNetwork> fmt::Display for DogecoinExtendedPublicKey<N> {
...
mod tests {
    ...
    fn test_from_extended_private_key<N: DogecoinNetwork>(
        ...
        let extended_private_key = DogecoinExtendedPrivateKey::<N>::from_str(extended_private_key).unwrap();
        let extended_public_key = DogecoinExtendedPublicKey::<N>::from_extended_private_key(&extended_private_key);
    ...
    fn test_derive<N: DogecoinNetwork>(
        ...
        let expected_extended_public_key2 =
            DogecoinExtendedPublicKey::<N>::from_str(&expected_extended_public_key2).unwrap();
    ...
    fn test_from_str<N: DogecoinNetwork>(
        ...
        let extended_public_key = DogecoinExtendedPublicKey::<N>::from_str(&extended_public_key).unwrap();
        assert_eq!(expected_public_key, extended_public_key.public_key.to_string());
        assert_eq!(expected_child_index, u32::from(extended_public_key.child_index));
        assert_eq!(expected_chain_code, hex::encode(extended_public_key.chain_code));
        assert_eq!(
    ...
    fn test_to_string<N: DogecoinNetwork>(expected_extended_public_key: &str) {
        let extended_public_key = DogecoinExtendedPublicKey::<N>::from_str(&expected_extended_public_key).unwrap();
        assert_eq!(expected_extended_public_key, extended_public_key.to_string());
    }
    ...
    mod test_invalid {
        ...
        fn from_str_invalid_secret_key() {
            let _result =
                DogecoinExtendedPublicKey::<N>::from_str(INVALID_EXTENDED_PUBLIC_KEY_SECP256K1_PUBLIC_KEY).unwrap();
        }

        #[test]
        #[should_panic(expected = "InvalidVersionBytes([4, 136, 178, 29])")]
        fn from_str_invalid_version() {
            let _result = DogecoinExtendedPublicKey::<N>::from_str(INVALID_EXTENDED_PUBLIC_KEY_NETWORK).unwrap();
        }

        #[test]
        #[should_panic(expected = "InvalidChecksum(\"5Nvot3\", \"5Nvot4\")")]
        fn from_str_invalid_checksum() {
            let _result = DogecoinExtendedPublicKey::<N>::from_str(INVALID_EXTENDED_PUBLIC_KEY_CHECKSUM).unwrap();
        }

        #[test]
        #[should_panic(expected = "InvalidByteLength(81)")]
        fn from_str_short() {
            let _result = DogecoinExtendedPublicKey::<N>::from_str(&VALID_EXTENDED_PUBLIC_KEY[1..]).unwrap();
        }
        ...
        fn from_str_long() {
            ...
            string.push('a');
            let _result = DogecoinExtendedPublicKey::<N>::from_str(&string).unwrap();
        }
    }
}
