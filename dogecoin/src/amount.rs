...
pub struct DogecoinAmount(pub i64);
...
impl Amount for DogecoinAmount {}

impl DogecoinAmount {
    /// The zero amount.
    pub const ZERO: DogecoinAmount = DogecoinAmount(0);
    /// Exactly one satoshi.
    pub const ONE_SAT: DogecoinAmount = DogecoinAmount(1);
    /// Exactly one bitcoin.
    pub const ONE_BTC: DogecoinAmount = DogecoinAmount(COIN);
    ...
    pub fn from_btc(btc_value: i64) -> Result<Self, AmountError> {
        let satoshis = btc_value * 10_i64.pow(Denomination::Dogecoin.precision());
    ...
    pub fn sub(self, b: DogecoinAmount) -> Result<Self, AmountError> {
        Self::from_satoshi(self.0 - b.0)
    }
}

impl fmt::Display for DogecoinAmount {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
mod tests {
    ...
    fn test_from_satoshi(sat_value: i64, expected_amount: DogecoinAmount) {
        let amount = DogecoinAmount::from_satoshi(sat_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_from_ubtc(ubtc_value: i64, expected_amount: DogecoinAmount) {
        let amount = DogecoinAmount::from_ubtc(ubtc_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_from_mbtc(mbtc_value: i64, expected_amount: DogecoinAmount) {
        let amount = DogecoinAmount::from_mbtc(mbtc_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_from_cbtc(cbtc_value: i64, expected_amount: DogecoinAmount) {
        let amount = DogecoinAmount::from_cbtc(cbtc_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_from_dbtc(dbtc_value: i64, expected_amount: DogecoinAmount) {
        let amount = DogecoinAmount::from_dbtc(dbtc_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_from_btc(btc_value: i64, expected_amount: DogecoinAmount) {
        let amount = DogecoinAmount::from_btc(btc_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_addition(a: &i64, b: &i64, result: &i64) {
        let a = DogecoinAmount::from_satoshi(*a).unwrap();
        let b = DogecoinAmount::from_satoshi(*b).unwrap();
        let result = DogecoinAmount::from_satoshi(*result).unwrap();
    ...
    mod valid_conversions {
        ...
        fn test_satoshi_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_satoshi(amounts.satoshi, DogecoinAmount(amounts.satoshi)));
        }
        ...
        fn test_ubtc_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_ubtc(amounts.micro_bit, DogecoinAmount(amounts.satoshi)));
        }
        ...
        fn test_mbtc_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_mbtc(amounts.milli_bit, DogecoinAmount(amounts.satoshi)));
        }
        ...
        fn test_cbtc_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_cbtc(amounts.centi_bit, DogecoinAmount(amounts.satoshi)));
        }
        ...
        fn test_dbtc_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_dbtc(amounts.deci_bit, DogecoinAmount(amounts.satoshi)));
        }
        ...
        fn test_btc_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_btc(amounts.bitcoin, DogecoinAmount(amounts.satoshi)));
        }
    }
