...
use crate::wordlist::DogecoinWordlist;
...
/// Represents a Dogecoin mnemonic
pub struct DogecoinMnemonic<N: DogecoinNetwork, W: DogecoinWordlist> {
...
impl<N: DogecoinNetwork, W: DogecoinWordlist> MnemonicCount for DogecoinMnemonic<N, W> {
...
impl<N: DogecoinNetwork, W: DogecoinWordlist> Mnemonic for DogecoinMnemonic<N, W> {
...
impl<N: DogecoinNetwork, W: DogecoinWordlist> MnemonicExtended for DogecoinMnemonic<N, W> {
...
impl<N: DogecoinNetwork, W: DogecoinWordlist> DogecoinMnemonic<N, W> {
...
impl<N: DogecoinNetwork, W: DogecoinWordlist> FromStr for DogecoinMnemonic<N, W> {
...
impl<N: DogecoinNetwork, W: DogecoinWordlist> fmt::Display for DogecoinMnemonic<N, W> {
...
mod tests {
    ...
    fn test_new_with_count<N: DogecoinNetwork, W: DogecoinWordlist>(word_count: u8) {
        let rng = &mut XorShiftRng::seed_from_u64(1231275789u64);
        let mnemonic = DogecoinMnemonic::<N, W>::new_with_count(rng, word_count).unwrap();
        test_from_phrase::<N, W>(&mnemonic.entropy, &mnemonic.to_phrase().unwrap());
    }

    fn test_from_phrase<N: DogecoinNetwork, W: DogecoinWordlist>(expected_entropy: &Vec<u8>, phrase: &str) {
        let mnemonic = DogecoinMnemonic::<N, W>::from_phrase(phrase).unwrap();
        assert_eq!(&expected_entropy[..], &mnemonic.entropy[..]);
        assert_eq!(phrase, mnemonic.to_phrase().unwrap());
    }

    fn test_to_phrase<N: DogecoinNetwork, W: DogecoinWordlist>(expected_phrase: &str, entropy: &Vec<u8>) {
        let mnemonic = DogecoinMnemonic::<N, W> {
    ...
    fn test_verify_phrase<N: DogecoinNetwork, W: DogecoinWordlist>(phrase: &str) {
        assert!(DogecoinMnemonic::<N, W>::verify_phrase(phrase));
    }

    fn test_to_seed<N: DogecoinNetwork, W: DogecoinWordlist>(
        expected_seed: &str,
        password: Option<&str>,
        mnemonic: DogecoinMnemonic<N, W>,
    ) {
        assert_eq!(expected_seed, &hex::encode(mnemonic.to_seed(password).unwrap()))
    }

    fn test_to_extended_private_key<N: DogecoinNetwork, W: DogecoinWordlist>(
    ...
    mod test_invalid {
        ...
        fn new_with_count_invalid_word_count() {
            let rng = &mut XorShiftRng::seed_from_u64(1231275789u64);
            let _mnemonic = DogecoinMnemonic::<N, W>::new_with_count(rng, INVALID_WORD_COUNT).unwrap();
        }

        #[test]
        #[should_panic(expected = "InvalidWord(\"abandoz\")")]
        fn from_phrase_invalid_word() {
            let _mnemonic = DogecoinMnemonic::<N, W>::from_phrase(INVALID_PHRASE_WORD).unwrap();
        }

        #[test]
        #[should_panic(expected = "InvalidWordCount(13)")]
        fn from_phrase_invalid_length() {
            let _mnemonic = DogecoinMnemonic::<N, W>::from_phrase(INVALID_PHRASE_LENGTH).unwrap();
        }
        ...
        fn from_phrase_invalid_checksum() {
            let _mnemonic = DogecoinMnemonic::<N, W>::from_phrase(INVALID_PHRASE_CHECKSUM).unwrap();
        }

        #[test]
        fn verify_invalid_phrase() {
            assert!(!DogecoinMnemonic::<N, W>::verify_phrase(INVALID_PHRASE_LENGTH));
        }
    }
}
