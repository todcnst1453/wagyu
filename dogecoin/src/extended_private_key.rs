...
use crate::derivation_path::DogecoinDerivationPath;
use crate::extended_public_key::DogecoinExtendedPublicKey;
...
/// Represents a Dogecoin extended private key
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DogecoinExtendedPrivateKey<N: DogecoinNetwork> {
    /// The address format
    pub(super) format: DogecoinFormat,
    ...
    /// The Dogecoin private key
    private_key: DogecoinPrivateKey<N>,
}

impl<N: DogecoinNetwork> ExtendedPrivateKey for DogecoinExtendedPrivateKey<N> {
    type Address = DogecoinAddress<N>;
    type DerivationPath = DogecoinDerivationPath<N>;
    type ExtendedPublicKey = DogecoinExtendedPublicKey<N>;
    ...
    fn derive(&self, path: &Self::DerivationPath) -> Result<Self, ExtendedPrivateKeyError> {
        ...
        for index in path.to_vec()?.into_iter() {
            ...
            let format = match path {
                DogecoinDerivationPath::BIP49(_) => DogecoinFormat::P2SH_P2WPKH,
...
impl<N: DogecoinNetwork> DogecoinExtendedPrivateKey<N> {
    /// Returns the format of the Dogecoin extended private key.
    pub fn format(&self) -> DogecoinFormat {
        self.format.clone()
    }
}

impl<N: DogecoinNetwork> FromStr for DogecoinExtendedPrivateKey<N> {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let format = DogecoinFormat::from_extended_private_key_version_bytes(&data[0..4])?;
        ...
        let private_key = DogecoinPrivateKey::from_secp256k1_secret_key(&SecretKey::parse_slice(&data[46..78])?, true);
...
impl<N: DogecoinNetwork> Display for DogecoinExtendedPrivateKey<N> {
...
mod tests {
    ...
    fn test_new<N: DogecoinNetwork>(
        ...
        path: &DogecoinDerivationPath<N>,
    ) {
        let extended_private_key =
            DogecoinExtendedPrivateKey::<N>::new(&hex::decode(seed).unwrap(), format, path).unwrap();
    ...
    fn test_derive<N: DogecoinNetwork>(
        ...
        let extended_private_key1 = DogecoinExtendedPrivateKey::<N>::from_str(expected_extended_private_key1).unwrap();
        let extended_private_key2 = extended_private_key1.derive(&path).unwrap();

        let expected_extended_private_key2 =
            DogecoinExtendedPrivateKey::<N>::from_str(&expected_extended_private_key2).unwrap();
    ...
    fn test_to_extended_public_key<N: DogecoinNetwork>(
    ...
    fn test_from_str<N: DogecoinNetwork>(
        ...
        let extended_private_key = DogecoinExtendedPrivateKey::<N>::from_str(expected_extended_private_key).unwrap();
        assert_eq!(expected_extended_private_key, extended_private_key.to_string());
        assert_eq!(
            expected_parent_fingerprint,
            hex::encode(extended_private_key.parent_fingerprint)
    ...
    fn test_to_string<N: DogecoinNetwork>(expected_extended_private_key: &str) {
        let extended_private_key = DogecoinExtendedPrivateKey::<N>::from_str(expected_extended_private_key).unwrap();
        assert_eq!(expected_extended_private_key, extended_private_key.to_string());
    }
    ...
    mod p2pkh_mainnet {
        ...
        const KEYPAIRS: [(&str, &str, &str, &str, &str, &str, &str, &str); 26] = [
            ...
            // Dogecoin Core Derivation Paths
            (
                "m/0'/0'",
                "747f302d9c916698912d5f70be53a6cf53bc495803a5523d3a7c3afa2afba94ec3803f838b3e1929ab5481f9da35441372283690fdcf27372c38f40ba134fe03",
                "2147483648",
            ...
            KEYPAIRS.iter().for_each(
                |(path, seed, child_index, secret_key, chain_code, parent_fingerprint, extended_private_key, _)| {
                    test_new::<N>(
                        ...
                        &DogecoinDerivationPath::from_str(path).unwrap(),
                    );
                },
            );
        }
    ...
    mod test_invalid {
        ...
        fn from_str_invalid_secret_key() {
            let _result =
                DogecoinExtendedPrivateKey::<N>::from_str(INVALID_EXTENDED_PRIVATE_KEY_SECP256K1_SECRET_KEY).unwrap();
        }

        #[test]
        #[should_panic(expected = "InvalidVersionBytes([4, 136, 173, 227])")]
        fn from_str_invalid_version() {
            let _result = DogecoinExtendedPrivateKey::<N>::from_str(INVALID_EXTENDED_PRIVATE_KEY_NETWORK).unwrap();
        }

        #[test]
        #[should_panic(expected = "InvalidChecksum(\"6vCfku\", \"6vCfkt\")")]
        fn from_str_invalid_checksum() {
            let _result = DogecoinExtendedPrivateKey::<N>::from_str(INVALID_EXTENDED_PRIVATE_KEY_CHECKSUM).unwrap();
        }

        #[test]
        #[should_panic(expected = "InvalidByteLength(81)")]
        fn from_str_short() {
            let _result = DogecoinExtendedPrivateKey::<N>::from_str(&VALID_EXTENDED_PRIVATE_KEY[1..]).unwrap();
        }
        ...
        fn from_str_long() {
            ...
            string.push('a');
            let _result = DogecoinExtendedPrivateKey::<N>::from_str(&string).unwrap();
        }
    }
}
